...
TODO: backward propagated waves.
...
Given the above results it is easy to find the action of $i\Delta$ on ordinary plane waves via linear combinations of the form 
\be
u_k(x)=u^1_k(x)+u^2_k(x)=U^{\LRDone}_k(x)+U^{\ULDone}_k(x) +U^{\LRDtwo}_k(x) +U^{\ULDtwo}_k(x).
\ee
